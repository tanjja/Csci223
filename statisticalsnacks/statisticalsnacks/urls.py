"""
URL configuration for statisticalsnacks project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import include, path
from django.http import HttpResponse
from django.conf.urls import handler404
from django.views.defaults import page_not_found

handler404 = page_not_found

def helloworld(request):
    return HttpResponse('<p>Hello, World! How are you today?</p>')
def banana(request):
    return HttpResponse('<p>Banana</p>')

urlpatterns = [
    path('admin/', admin.site.urls), # http://localhost:8000/admin
    path('', helloworld), # http://localhost:8000/
    # create an endpoint that results in the browser rendering the word Banana
    # the url of the endpoint should be http://localhost:8000/banana
    path('banana/', banana),
    path('webkiosk/', include('webkiosk.urls')), # http://localhost:8000/webkiosk
]
